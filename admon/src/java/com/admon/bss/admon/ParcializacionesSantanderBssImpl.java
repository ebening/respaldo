...
public class ParcializacionesSantanderBssImpl extends BaseBss implements ParcializacionesSantanderBss {
 ...
 * método setter por cada variable que se anexa al final de la clase. */
    ...
    public ParcializacionesSantanderBssImpl() {
    }
 ...
 * @return Regresa una lista con los id's de los registros guardados.
    ...
    public List<Long> save(Integer userId, List<ParcializacionSantander> objList) {
        ...
        for (ParcializacionSantander obj : objList) {
            ...
            AfiliacionSantander afiliacion = afiliacionSantanderBss.findByName(obj.getAfiliacion());
            afiliacion.setTieneParcializacion(SI);
            afiliacionSantanderBss.update(userId, Arrays.asList(afiliacion));
...
//        Long idParcializacion=0L;
        ParcializacionSantander obj =new ParcializacionSantander();
        for (Long id : idList) {
             obj = findById(id);
            parcializacionSantanderDAO.delete(obj);
//            idParcializacion=id;
        }
        if(obj !=null && obj.getAfiliacion() != null){
            
            DetachedCriteria criteria = createDetachedCriteria();
            criteria.add(Expression.eq("afiliacion", obj.getAfiliacion()));
 ...
 * @return Regrega un objeto <b>Grid</b> con los datos a mostrar en la pantalla.
    ...
    public Grid findByCriteria(int displayedPage, int maxResult, String order,
      ...
      /*  if (aplicacion.getCreacionFechaInicial() != null) {
        ...
        List<ParcializacionSantander> afiliacionSantanderList = findByCriteriaLimit(criteria,
                maxResult * (displayedPage - 1), maxResult);
        grid.setGrid(resolveDescription(afiliacionSantanderList));
