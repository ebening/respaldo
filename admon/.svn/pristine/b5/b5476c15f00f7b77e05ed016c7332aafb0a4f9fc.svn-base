 ...
 * registro con ese nombre regresa un <b>true</b>.
     */
    @Override
    public boolean isValidoNombre(Integer clasificacionContraId, String nombre) {
        return !(findByCriteria(createDetachedCriteria().add(
                Expression.eq("nombre", nombre.trim())).add(Expression.eq("clasificacionContraId", clasificacionContraId))).size() >= 1);
    }
            ...
            for (SubclasificacionContra subclasificacionContra : list) {
                ...
                if (subclasificacionContra.getModificacionUsuario() != null) {
                    for (Usuario o : usuarioList) {
                        if (o.getUsuarioId().intValue()== subclasificacionContra.getModificacionUsuario().intValue()) {
                ...
                if (subclasificacionContra.getClasificacionContraId() != null) {
                    for (ClasificacionContra o : clasificacionContraList) {
                        if ( o.getClasificacionContraId().intValue() == subclasificacionContra.getClasificacionContraId().intValue()) {
