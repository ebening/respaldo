...
var utilMisc = {
    ...
    getDialogOrganizaciones : function ( objectJS, dialogId ) {
        var listOrganizaciones=[];
        jQuery( dialogId ).find( 'input:checkbox' ).each( function() {
            if ( jQuery( this ).is( ':checked' ) ) {
                var id = jQuery( this )[0].nextSibling.nodeValue;
                var valor = jQuery( this ).val();
                if(valor != -1){
                    listOrganizaciones.push(valor);
                }
            }
        });
        // Llamar a la función exportarPDF que se encarga de iniciar la generación del PDF
        eval( objectJS + '("' + listOrganizaciones + '")');
    },
    /**
...
//        jquery_dialogo += "show         : 'highlight',";
    ...
    selectedAllOrg: function (source){
        checkboxes = document.getElementsByName('org');
        for(var i=0, n=checkboxes.length;i<n;i++) {
          checkboxes[i].checked = source.checked;
        }
    },
    setCustomDialogOrganizaciones : function (dialogId,listObject,object,proprety) {
        // Titulo del mensaje
        var title = "Seleccionar organizaciones";
        // Empezar a construir el contenido
        var content = '';
        content += '<div style="border-bottom: 1px solid #DDDDDD; padding-bottom: 5px; margin-bottom: 5px;">';
        content += '<b>&iquest;A qu&eacute; organizaci&oacute;n desea agregarle la restricci&oacute;n?</b><br>';
        content += '</div>';
        content += '<input type="checkbox" value ="-1" onClick="utilDialog.selectedAllOrg(this)" checked /> <b> Seleccionar Todos </b><br/>';
        content += '<div style="height: 170px; overflow-y: scroll;">';
        for ( var i = 0; i < listObject.length; i++ ) {
            obj = listObject[i];
            content += "<div class='row'>";
            content += "<div class='col-sm-6'>";
            content += "<input type='checkbox' name='org' value='" + listObject[i].organizacionId + "' checked>" +obj[proprety] +"<br>";
            content += "</div>";
            i++;
        ...
        // Calls
        var calls = [
        "Aceptar@utilMisc.getDialogOrganizaciones('" + object+ "','"  + dialogId + "');",
        "Cancelar@" ];
...
var utilGrid = {
    ...
    gridUpdate : function( gridId, gridCurrentPage, gridRowsByPage, data ) {
        ...
        for( var i = 0; i < grid.length; i++ ) {
            ...
            var estatusBitacoracheck_css = 'ui-icon ui-icon-check ui-icon-center';
            var estatusBitacoraclose_css = 'ui-icon ui-icon-close ui-icon-center';
            
            var estatusBitacoraSi_title = 'Desplegado';
            var estatusBitacoraNo_title = 'No Desplegado';
            ...
            if (gridId === '#catalogoGrid'||gridId === '#catalogoParametroGrid'||gridId === '#catalogoParametroLenguajeGrid'){
                if(gridId === '#catalogoGrid'){
                    ...
                    var eliminarCatalogo_onclick = entity + 'JS.prepareToRemove(' + grid[ i ]['catalogoPK']['catalogoId'] + ',' + grid[ i ]['catalogoPK']['organizacionId'] + ')';
                     // Funcionalidad para Restricciones
                    var restriccion_title = 'Restriccion';
                    var restriccion_onclick = entity + 'JS.showRestricciones('+  grid[ i ]['catalogoPK']['catalogoId'] + ',' + grid[ i ]['catalogoPK']['organizacionId'] +  ',' + grid[ i ]['generico'] +')';
                    ...
                    // Eliminar
                    var eliminarCatalogoParametro_onclick = entity + 'JS.prepareToRemove(' + grid[ i ]['catalogoParametroPK']['catalogoParametroId'] + ',' + grid[ i ]['catalogoId'] + ',' + grid[ i ]['catalogoParametroPK']['organizacionId'] + ')';
            ...
            if(gridId === '#perfilAplicacionGrid'){
                    // Editar
                    var editarPerfilAplicacion_onclick = entity + 'JS.findPerfilAplicacionById(' + grid[ i ]['aplicacionId'] + ',' + grid[ i ]['seguridadPerfilId'] + ')';
                    // Eliminar
                    var eliminarPerfilAplicacion_onclick = entity + 'JS.prepareToRemove(' + grid[ i ]['aplicacionId'] + ',' + grid[ i ]['seguridadPerfilId'] + ')';
            }
            //Parametro Lenguaje
            // Editar
            var editarParametroLenguaje_onclick = entity + 'JS.findParametroById(' + grid[ i ]['parametroId'] + ',' + grid[ i ]['lenguajeId'] + ')';
            // Eliminar
            var eliminarParametroLenguaje_onclick = entity + 'JS.prepareToRemove(' + grid[ i ]['parametroId'] + ',' + grid[ i ]['lenguajeId'] + ')';
            ...
            //if ( grid[ i ]['estatusId'] == 'Activo' || grid[ i ]['estatusId'] == 5 ) {
             //   grid[i].StateSeguridad = "<span class = '" + activo_css + "' title = '" + activo_title + "' style = 'cursor:pointer' onclick = '" + activoSeguridad_onclick + "'></span>";
            //} else {
            //    grid[i].StateSeguridad = "<span class = '" + inactivo_css + "' title = '" + inactivo_title + "' style = 'cursor:pointer' onclick = '" + inactivoSeguridad_onclick + "'></span>";
           // }
...
//Funcionalidad para Catalogo
            if (gridId === '#catalogoGrid'||gridId === '#catalogoParametroGrid'||gridId === '#catalogoParametroLenguajeGrid' ||gridId === '#parametroGrid'){
                ...
                }else if(gridId === '#parametroGrid'){
                    grid[i].EditParametro = "<span class = '" + editar_css + "' title = '" + editar_title + "' style = 'cursor:pointer' onclick = '" + editarParametroLenguaje_onclick + "'></span>";
                    grid[i].RemoveParametro = "<span class = '" + eliminar_css + "' title = '" + eliminar_title + "' style = 'cursor:pointer' onclick = '" + eliminarParametroLenguaje_onclick + "'></span>";
                }
            }
            if(gridId === '#perfilAplicacionGrid'){
                    grid[i].EditPerfilAplicacion = "<span class = '" + editar_css + "' title = '" + editar_title + "' style = 'cursor:pointer' onclick = '" + editarPerfilAplicacion_onclick + "'></span>";
                    grid[i].RemovePerfilAplicacion = "<span class = '" + eliminar_css + "' title = '" + eliminar_title + "' style = 'cursor:pointer' onclick = '" + eliminarPerfilAplicacion_onclick + "'></span>";
            }
            if(gridId === '#catalogoGrid'){
                grid[i].Restriccion = "<span class = '" + editar_css + "' title = '" + restriccion_title + "' style = 'cursor:pointer' onclick = '" + restriccion_onclick + "'></span>";
            }
            if(gridId === '#bitacoraGrid'){
                if ( grid[ i ].estatusId == 1) {
                    grid[ i ].estatusId = "<span class = '" + estatusBitacoracheck_css + "' title = '" + estatusBitacoraSi_title + "' ></span>";
                } else {
                    grid[ i ].estatusId = "<span class = '" + estatusBitacoraclose_css + "' title = '" + estatusBitacoraNo_title + "' ></span>";
                }
...
//Funcionalidad para Catalogo
            ...
            if (gridId === '#afiliacionSantanderGrid'){
                if ( grid[ i ]['valida3ds'] == 1 ) {
                        grid[i].valida3ds = 'Si';
                } else {
                        grid[i].valida3ds = 'No';
                }
                if ( grid[ i ]['validaSdos'] == 1 ) {
                        grid[i].validaSdos = 'Si';
                } else {
                        grid[i].validaSdos = 'No';
                }
                if ( grid[ i ]['amex'] == 1 ) {
                        grid[i].amex = 'Si';
                } else {
            ...
            if (gridId === '#estadoCuentaGrid'){
//                if (grid[ i ]['monto']) {
//                    var valor = "$ "+grid[i].monto;
//                    grid[i].monto = valor;
//                }
//                if (grid[ i ]['cargoXServicio']) {
//                    var valor = "$ "+grid[i].cargoXServicio;
//                    grid[i].cargoXServicio = valor;
//                }
//                if (grid[ i ]['total']) {
//                    var valor = "$ "+grid[i].total;
//                    grid[i].total = valor;
//                }
//                if (grid[ i ]['comisionTaasgo']) {
//                    var valor = "$ "+grid[i].comisionTaasgo;
//                    grid[i].comisionTaasgo = valor;
//                }
    ...
    gridUpdateFila : function( gridId, gridCurrentPage, gridRowsByPage, data ,condicion) {
        var entity = utilString.removeSharp( utilString.removeGrid( gridId ) );
        var grid = data.grid;
        jQuery( gridId ).clearGridData();
        jQuery( gridId ).setGridParam({
    ...
    /**
        ...
        for( var i = 0; i < grid.length; i++ ) {
            ...
            for (var idx = 0; idx < propiedades.length; idx ++) {
                var attr = propiedades[ idx ] + '';
                // Fechas
                grid[ i ][ attr ]=utilGrid.setFormatFechaFila(grid[ i ][ attr ],attr);
                if ( utilString.endsWith( attr, 'Id' ) ) {
                    if (attr==condicion) {
                        grid[ i ][ attr ] = grid[ i ][ attr.substring( 0, attr.length - 2 ) ];
                    }
                }
                if ( attr == 'modificacionUsuario' ) 
                    grid[ i ][ attr ] = grid[ i ].usuarioModificacion;
            }
            grid[ i ] = utilGrid.setFormatEditFila(i+1,grid[ i ],entity);
            jQuery( gridId ).addRowData( i + 1, grid[ i ] );
        }
        // Mensaje loading
        jQuery( '.loading' ).css( 'display', 'none' );
    },
    setFormatEditFila:function(filaId,object,entity){
        // Editar
        var editar_css = 'ui-icon ui-icon-pencil ui-icon-center';
        var editar_title = 'Editar';
        var editar_onclick = entity + 'JS.findByFilaId(' + (filaId) + ')';
        // Eliminar
        var eliminar_css = 'ui-icon ui-icon-trash ui-icon-center';
        var eliminar_title = 'Eliminar';
        var eliminar_onclick = entity + 'JS.prepareToRemove(' + (filaId) + ')';

        object.Edit = "<span class = '" + editar_css + "' title = '" + editar_title + "' style = 'cursor:pointer' onclick = '" + editar_onclick + "'></span>";
        object.Remove = "<span class = '" + eliminar_css + "' title = '" + eliminar_title + "' style = 'cursor:pointer' onclick = '" + eliminar_onclick + "'></span>";
        object.filaId=filaId;
        return object;
    },
    setFormatFechaFila:function(object,propiedad){
         // Fechas
        if ( utilString.containsIgnoreCase( propiedad, 'hora' ) ) {
            if ( object  != null ) {
                object = utilForm.getValorDateTimeFixedYear(object);
            }
        } else if ( utilString.containsIgnoreCase( propiedad, 'fecha' ) || 
            utilString.containsIgnoreCase( propiedad, 'date' ) ) {
            if ( object  != null ) {
                object = jQuery.datepicker.formatDate('dd/mm/y', 
                    new Date( object ) );
            }
        }
        return object;
    ...
    gridGetSelectedRows : function ( gridId ) {
        ...
        for (var i = 0 ; i < selectedRows.length ; i++ ) {
            ...
            var id = "";
            if(gridId === '#catalogoGrid'){
                id = registro[ 'catalogoPK.' + entity + "Id" ];
            } else {
                id = registro[ entity + "Id" ];
            }
            if(gridId === '#catalogoGrid'){
                listaId[ listaId.length ] = id;//{catalogoPK: registro['catalogoPK']['catalogoId'], organizacionId: registro['catalogoPK']['organizacionId']}; 
            } else if(gridId === '#catalogoParametroGrid'){
    ...
    setHideElementsTable : function ( gridPagerId,arrayElements) {
        for (var i = 0; i < arrayElements.length; i++) {
        switch(arrayElements[i]) {
            case 'refresh':
                 jQuery('#pg_'+gridPagerId).find('#grid-action-actualizar').hide().next().hide();
                 break;
             case 'search':
                 jQuery('#pg_'+gridPagerId).find('#grid-action-buscar').hide().next().hide();
                 break;
             case 'export':
                 jQuery('#pg_'+gridPagerId).find('#grid-action-exportar').hide().next().hide();
                 break;
             case 'activate':
                  jQuery('#pg_'+gridPagerId).find('#grid-action-activar-inactivar').hide().next().hide();
                 break;
             case 'delete':
                  jQuery('#pg_'+gridPagerId).find('#grid-action-eliminar-masivo').hide().next().hide();
                 break;
             default:
    }
}
...
var utilObject = {
    ...
    buildDWRObjectListByGridCheck : function( gridId, objetoDWR) {
        var selectedRows = jQuery( gridId ).jqGrid( 'getGridParam', 'selarrrow' );
        var objectList = [];
        for (var i = 0 ; i < selectedRows.length ; i++ ) {
            var registro = jQuery(gridId).getRowData( selectedRows[ i ] );
    ...
    buildDWRObjectByGridObject : function( objetoDWR, objetoGrid ) {
        ...
        for ( var i = 0; i < properties.length; i++ ) {
            if ( properties[ i ].indexOf( 'fecha' ) >= 0 ) {
                ...
                if(( typeof objetoDWR[properties[ i ] ] === 'object') 
                      && (properties[ i ]  === 'catalogoParametroPK'||properties[ i ]  === 'catalogoParametroLenguajePK' )){//Aplica solo para Catalogo){
                    var objetoDWRSub = objetoDWR[properties[ i ] ];
                    if(typeof  objetoDWRSub === "undefined" || objetoDWRSub === null){
                        if(properties[ i ]  === 'catalogoParametroPK'){
                            objetoDWRSub = {catalogoParametroId: objetoGrid['catalogoParametroPK.catalogoParametroId'], organizacionId:  objetoGrid['catalogoParametroPK.organizacionId']};                         
                        }else if(properties[ i ]  === 'catalogoParametroLenguajePK'){
                            objetoDWRSub = {catalogoParametroLenguajeId: objetoGrid['catalogoParametroLenguajePK.catalogoParametroLenguajeId'], organizacionId:  objetoGrid['catalogoParametroLenguajePK.organizacionId']};                         
                        }
                    }
                    objetoDWR[properties[ i ] ] = objetoDWRSub;
                }else{
    ...
    getFuncionalidadPantalla: function(patron,actionName){
        var pantalla = jQuery('.title-text:first').text(); 
//        pantalla = pantalla.replace(patron, nuevoValor);
         var criteriaExample = new PerfilPaginaPantallaDWR();
         criteriaExample.pantalla = pantalla;
         criteriaExample.patern = patron;
         criteriaExample.actionName = actionName;
         PerfilPaginaPantallaDWR.getFuncionalidadPantalla(criteriaExample, utilObject.getFuncionalidadPantallaCallback);
    },
    getFuncionalidadPantallaCallback: function(data){
        //console.info(data);
        //console.info("Nombre de Pantalla= " + data[0].pantalla);
        // 1=CREAR, 2=EDITAR, 3=ELIMINAR,  4=CONSULTAR,  5=EXPORTAR, 6=CAMBIAR ESTATUS,  7=BUSCAR, 8=Desplegar
        var funcionalidadesAocultar = [1,2,3,4,5,6,7,8];
        var pagerList = [];
        var pantalla = data[0].pantalla;
        var col = "";
        if(data[0].actionName == 'SeguridadPerfilAction'){
            col = "#seguridadPerfilGrid";
        }
        if(data[0].actionName == 'ClasificacionContraAction'){
            col = "#clasificacionContraGrid";
        }if(data[0].actionName == 'SubclasificacionContraAction'){
            col = "#subclasificacionContraGrid";
        }if(data[0].actionName == 'NombreContraAction'){
            col = "#nombreContraGrid";
        }if(data[0].actionName == 'EstadoCuentaAction'){
            col = "#estadoCuentaGrid";
        }if(data[0].actionName == 'CatalogoAction'){
            col = "#catalogoGrid";
        }else{
            col = "#"+ data[0].pantalla.toLowerCase() +"Grid";
        }
        for ( var i = 0; i < data.length; i++ ) {
            for ( var j = 0; j < funcionalidadesAocultar.length; j++ ){
                if(data[i].funcionalidadId==funcionalidadesAocultar[j]){
                      funcionalidadesAocultar.splice(j, 1);
                      break;
                }
            }
        }
        for ( var i = 0; i < funcionalidadesAocultar.length; i++ ){
            if(funcionalidadesAocultar[i]==1){ //1=CREAR
                if(data[0].actionName == 'CatalogoAction'){
                    jQuery('#btnNewCatalogo').hide();
                    jQuery('#btnSaveCatalogo').hide();
                }else{
                    jQuery('#btnNew'+pantalla).hide();
                    jQuery('#btnSave'+pantalla).hide();
                    jQuery('#btnAddOrganizacionUsuario').hide();
                    jQuery('#btnAddOrganizacionCredencial').hide();
                    jQuery('#btnNewSubclasificacionContra').hide();
                    jQuery('#btnSaveSubclasificacionContra').hide();
                    jQuery('#btnNewNombreContra').hide();
                    jQuery('#btnSaveNombreContra').hide();
                    jQuery('#btnBuscarEstadoCuenta').hide();
                    jQuery('#btnExportarEstadoCuenta').hide();
                    jQuery('#btnNewPerfilPagina').hide();
                    jQuery('#btnSavePerfilPagina').hide();
                    jQuery('#btnNewAfiliacionSantander').hide();
                    jQuery('#btnSaveAfiliacionSantander').hide();
                    jQuery('#btnNewAfiliacionBanamex').hide();
                    jQuery('#btnSaveAfiliacionBanamex').hide();
                    jQuery('#btnNewAfiliacion').hide();
                    jQuery('#btnSaveAfiliacion').hide();
                    jQuery('#btnNewParcializacionBanamex').hide();
                    jQuery('#btnAddParcializacionBanamex').hide();
                    jQuery('#btnNewParcializacionSantander').hide();
                    jQuery('#btnAddParcializacionSantander').hide();
                    jQuery('#btnNewTerminalBanamex').hide();
                    jQuery('#btnAddTerminalBanamex').hide();
                }
         }
            if(funcionalidadesAocultar[i]==2){ //2=EDITAR
                if(data[0].actionName == 'CatalogoAction'){
                    jQuery('#catalogoGrid').hideCol( 'EditCatalogo' );
                }else{
                    jQuery( col ).hideCol( 'Edit' );
                }
            }
            if(funcionalidadesAocultar[i]==3){ //3=ELIMINAR
                jQuery(col).hideCol( 'Remove' );
                jQuery('#parametroGrid').hideCol( 'RemoveParametro' );
                jQuery('#seguridadPerfilRolGrid').hideCol( 'Remove' );
                jQuery('#organizacionUsuarioGrid').hideCol( 'Remove' );
                jQuery('#organizacionCredencialGrid').hideCol( 'Remove' );
                jQuery('#afiliacionSantanderGrid').hideCol( 'Remove' );
                jQuery('#afiliacionBanamexGrid').hideCol( 'Remove' );
                jQuery('#configuracionFormaPagoGrid').hideCol( 'Remove' );
                jQuery('#parcializacionSantanderGrid').hideCol( 'Remove' );
                jQuery('#parcializacionBanamexGrid').hideCol( 'Remove' );
                jQuery('#catalogoGrid').hideCol( 'RemoveCatalogo' );
                
                pagerList[3] = "delete";
            }
            if(funcionalidadesAocultar[i]==4){ //4=CONSULTAR
                jQuery('#btnBuscarEstadoCuenta').hide();
            }
            if(funcionalidadesAocultar[i]==5){ //5=EXPORTAR
                pagerList[2] = "export";
                jQuery('#btnExportarEstadoCuenta').hide();
            }
            if(funcionalidadesAocultar[i]==6){ //6=CAMBIAR
                jQuery(col).hideCol( 'StateEventos' );
                jQuery(col).hideCol( 'StateSC' );
                jQuery(col).hideCol( 'State' );
                jQuery(col).hideCol( 'StateSeguridad' );
                jQuery(col).hideCol( 'StateCatalogo' );
                pagerList[0] = 'activate';
                jQuery('#organizacionUsuarioGrid').hideCol( 'Estatus' );
                jQuery('#organizacionCredencialGrid').hideCol( 'Estatus' );
            }
            if(funcionalidadesAocultar[i]==7){ //7=BUSCAR
               pagerList[1] = "search";
            }
            if(funcionalidadesAocultar[i]==8){ //0=Desplegar
                if(data[0].actionName == 'CatalogoAction'){
                    jQuery('#btnDesplegar').hide();
                }else{
                    jQuery('#btnDesplegar'+pantalla).hide();
                    jQuery('#btnCreaOrganizacionGenerarNoValidation').hide();
                }
               
            }
        }
        if(data[0].actionName == 'SeguridadPerfilAction'){
            pantalla = "seguridadPerfil";
            utilGrid.setHideElementsTable("seguridadPerfilRol" + 'GridPagerId',pagerList);
        }
        if(data[0].actionName == 'AfiliacionSantanderAction'){
            utilGrid.setHideElementsTable('afiliacionSantanderGridPagerId',pagerList);
        }if(data[0].actionName == 'AfiliacionBanamexAction'){
            utilGrid.setHideElementsTable('afiliacionBanamexGridPagerId',pagerList);
            utilGrid.setHideElementsTable('parcializacionBanamexGridPagerId',pagerList);
        } if(data[0].actionName == 'ConfiguracionFormaPagoAction'){
            utilGrid.setHideElementsTable('configuracionFormaPagoGridPagerId',pagerList);
        } if(data[0].actionName == 'CatalogoAction'){
            utilGrid.setHideElementsTable('catalogoGridPagerId',pagerList);
            utilGrid.setHideElementsTable('catalogoParametroGridPagerId',pagerList);
        }
        if(data[0].actionName == 'ClasificacionContraAction'){
            pantalla = "clasificacionContra";
        }if(data[0].actionName == 'SubclasificacionContraAction'){
             pantalla = "subclasificacionContra";
        }if(data[0].actionName == 'NombreContraAction'){
            pantalla = "nombreContra";
        }if(data[0].actionName == 'EstadoCuentaAction'){
            pantalla = "estadoCuenta";
        }else {
            pantalla = pantalla.toLowerCase();
        }
        if(data[0].actionName == 'CatalogoAction'){
            
        }else{
            utilGrid.setHideElementsTable(pantalla + 'GridPagerId',pagerList);
        }
        
        if(data[0].actionName == 'PerfilPaginaAction'){
...
function getDateFormat(valor){
	if(typeof valor == 'string'){
		if (valor.match(/^(\d{1,2})\/(\d{1,2})\/(\d{2})$/)) {
			return 'dd/mm/y';
		} else if (valor.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/)) {
			return 'dd/mm/yy';
		}
	}
	return 'dd/mm/yy';
