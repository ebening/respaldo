...
import com.admon.dao.admon.TerminalBanamexDAO;
...
public class TerminalBanamexBssImpl extends BaseBss implements TerminalBanamexBss {

    /* Inyección de dependencias con Spring. Estas dependencias se
 * configuran en el applicationContext.xml, y además necesitan un
 * método setter por cada variable que se anexa al final de la clase. */
    private TerminalBanamexDAO terminalBanamexDAO;
 ...
 * @return Regresa una lista con los id's de los registros guardados.
    ...
    public List<Integer> save(Integer userId, List<TerminalBanamex> afiliacionBanamexList) {
        ...
        for (TerminalBanamex terminal : afiliacionBanamexList) {
            terminal.setModificacionUsuario(userId);
            terminal.setCreacionUsuario(userId);
            terminal.setCreacionFecha(new Timestamp(new Date().getTime()));
            terminal.setModificacionFecha(new Timestamp(new Date().getTime()));
            terminal.setTerminalBanamexId(terminalBanamexDAO.getSecuencia().toString());
            savedList.add(terminalBanamexDAO.save(terminal));
            AfiliacionBanamex afiliacion = afiliacionBanamexBss.findByName(terminal.getAfiliacionBanamex());
            afiliacion.setTieneTerminales("SI");
 ...
 * @param userId Es el id del usuario que realizó la operación. * @param afiliacionBanamexList Lista de registros que se eliminarán en la tabla de <b>AFILIACION_BANAMEX_PLAN</b> en la BD.
    ...
    public void delete(Integer userId, List<Integer> afiliacionBanamexList) {
        TerminalBanamex terminal=null;
        for (Integer afiliacion : afiliacionBanamexList) {
            terminal = findByField(afiliacion);
            terminal.setModificacionUsuario(userId);
            terminal.setModificacionFecha(new Timestamp(new Date().getTime()));
            terminalBanamexDAO.delete(terminal);
        }
         if(terminal !=null){
            DetachedCriteria criteria = createDetachedCriteria();
            criteria.add(Expression.eq("afiliacionBanamex", terminal.getAfiliacionBanamex()));
            int valor = findSizeByCriteria(copy(criteria));
            if(valor <= 0){
                AfiliacionBanamex afiliacion = afiliacionBanamexBss.findByName(terminal.getAfiliacionBanamex());
                afiliacion.setTieneTerminales("NO");
                afiliacionBanamexBss.update(userId, Arrays.asList(afiliacion));
            }
        }
    }
 ...
 * registro con ese nombre regresa un <b>true</b>.
    ...
    public boolean isValidoAfiliacionBanamex(TerminalBanamex terminalBanamex) {
        //DetachedCriteria criteria = createDetachedCriteria();
        //criteria.add(Expression.eq("afiliacion", terminalBanamex.getAfiliacionBanamex()));
        return true;
    }
 ...
 * @return Regresa un objeto tipo <b>AFILIACION_BANAMEX_PLAN</b> con la información de la consulta.
    ...
    public TerminalBanamex findByField(Integer id) {
        DetachedCriteria criteria = createDetachedCriteria();
        criteria.add(Expression.eq("terminalBanamexId", id.toString()));
        return findByCriteria(criteria).get(0);
    }
    
    @Override
    public TerminalBanamex findById(Integer afiliacionBanamexId) {
        TerminalBanamex afiliacionBanamex = findByField(afiliacionBanamexId);
 ...
 * @return Regrega un objeto <b>Grid</b> con los datos a mostrar en la pantalla.
    ...
    public Grid findByCriteria(int displayedPage, int maxResult, String order,
        ...
        List<TerminalBanamex> afiliacionBanamexList = findByCriteriaLimit(criteria,
    ...
    public TerminalBanamex findFirst() {
        List<TerminalBanamex> afiliacionBanamexList = findByCriteria(createDetachedCriteria());
    ...
    public List<TerminalBanamex> resolveDescription(List<TerminalBanamex> list) {
        if (!list.isEmpty()) {
            List<Usuario> usuarioList = usuarioBss.findAll();
            for (TerminalBanamex afiliacionBanamex : list) {
    ...
    public Integer findSizeByCriteria(DetachedCriteria criteria) {
        return terminalBanamexDAO.findSizeByCriteria(generalizarCriteria(criteria));
    }

    public List<TerminalBanamex> findByCriteriaLimit(DetachedCriteria criteria, Integer from, Integer to) {
        return terminalBanamexDAO.findByCriteriaLimit(generalizarCriteria(criteria), from, to);
    }
 ...
 * <b>criteria</b>.
    ...
    public List<TerminalBanamex> findByCriteria(DetachedCriteria criteria) {
        return terminalBanamexDAO.findByCriteria(generalizarCriteria(criteria));
    }
    ...
    public List<TerminalBanamex> findByCriteriaIgnorePrivacy(TerminalBanamex afiliacionBanamex) {
        DetachedCriteria criteria = createDetachedCriteria();
        criteria.add(Example.create(afiliacionBanamex));
        return terminalBanamexDAO.findByCriteria(criteria);
    }

    public DetachedCriteria generalizarCriteria(DetachedCriteria criteria) {
        resolvePrivacy(criteria, TerminalBanamexBss.class);
        return criteria;
    }
 ...
 * @return Regresa un objeto DetachedCriteria.
    ...
    public DetachedCriteria createDetachedCriteria() {
        return terminalBanamexDAO.createDetachedCriteria();
    }
    /*
 ...
 * Setter.
    ...
    /**
     * @param terminalBanamexDAO the terminalBanamexDAO to set
     */
    public void setTerminalBanamexDAO(TerminalBanamexDAO terminalBanamexDAO) {
        this.terminalBanamexDAO = terminalBanamexDAO;
    }
