 ...
 * comisión por organización en la tabla, sin importar el valor.
 ...
 * registro con ese nombre regresa un <b>true</b>.
    ...
    public boolean isValidoComision(int organizacionId,Double valorComision) {
        ...
        //En caso de que se tenga que validar la comisión por organización tomando en cuenta el valor descomentar
    ...
    * @param comision Es la comisión se desea buscar en la tabla de la BD.
    * @return Regresa <b>false</b> no existe <b>true</b> si existio y ya
    * se actualizo el estatus
        */
    @Override
    public boolean exists(Integer userId, Comision comision) {
        DetachedCriteria criteria = createDetachedCriteria();
        criteria.add(Expression.eq("organizacionId", comision.getOrganizacionId()));
        criteria.add(Expression.eq("clasificacionContraId", comision.getClasificacionContraId()));
        criteria.add(Expression.eq("subclasificacionContraId", comision.getSubclasificacionContraId()));
        criteria.add(Expression.eq("nombreContraId", comision.getNombreContraId()));
        criteria.add(Expression.eq("valor", comision.getValor()));
        criteria.add(Expression.eq("eliminadoId", ELIMINADO));
        //validar si existe el registro
        List<Comision> list = comisionDAO.findByCriteria(criteria);
        //si es diferente de null existe el registro por tanto solo se le cambiara el estatus de eliminado
        if(list.size() >0){
            comision = list.get(0);
            comision.setEliminadoId(NOELIMINADO);
            //se actualiza el estatus
            List<Integer> ids = update(userId, Arrays.asList(comision));
            // si se actualizo 
            if(ids.size()>0)
                return true;
            return false;
 ...
 * @return Regrega un objeto <b>Grid</b> con los datos a mostrar en la pantalla.
    ...
    public List<Comision> resolveDescription(List<Comision> list) {
        ...
        if (!list.isEmpty()) {
            ...
            List<Organizacion>  organizacionList=organizacionBss.findByCriteriaIgnorePrivacy(new Organizacion());
            List<ClasificacionContra> clasificacionContraList = clasificacionContraBss.findByCriteriaIgnorePrivacy(new ClasificacionContra());
            List<SubclasificacionContra> subclasificacionContraList = subclasificacionContraBss.findByCriteriaIgnorePrivacy(new SubclasificacionContra());
            List<NombreContra> nombreContraList = nombreContraBss.findByCriteriaIgnorePrivacy(new NombreContra());
            ...
            for (Comision comision : list) {
                ...
                if (comision.getOrganizacionId() != null) {
                    for (Organizacion o : organizacionList) {
                         if (o.getOrganizacionId().intValue() == comision.getOrganizacionId().intValue()) {
                            comision.setOrganizacion(o.getNombre());
                            break;
                        }
                    }
                }
                if(comision.getOrganizacion().equals("")){
                   continue;
                }
                ...
                comision.setClasificacionContra("");
                if (comision.getClasificacionContraId() != null) {
                    for (ClasificacionContra o : clasificacionContraList) {
                        if (o.getClasificacionContraId().intValue() == comision.getClasificacionContraId().intValue()) {
                            comision.setClasificacionContra(o.getNombre());
                            break;
                        }
                    }
                }
                if(comision.getClasificacionContra().equals("")){
                   continue;
                }
                ...
                if (comision.getSubclasificacionContraId() != null) {
                    for (SubclasificacionContra o : subclasificacionContraList) {
                        if (o.getSubclasificacionContraId().intValue() == comision.getSubclasificacionContraId().intValue()) {
                            comision.setSubclasificacionContra(o.getNombre());
                            break;
                        }
                    }
                }
                if(comision.getSubclasificacionContra().equals("")){
                    continue;
                }
                ...
                if(comision.getNombreContra().equals("")){
                    continue;
                }
