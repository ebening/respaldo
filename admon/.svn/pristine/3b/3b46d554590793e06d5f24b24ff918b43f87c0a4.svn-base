jQuery(document).ready(function () {
    ...
    utilObject.getFuncionalidadPantalla("es","ComisionAction");
    ...
    if (jQuery('#gridVisibleComision').val() == 'true') {
        jQuery('#comisionGrid').jqGrid({
            ...
            colNames: [
                ...
                'Modificar', 
                jQuery('#comision_eliminadoIdText').html(),],
            colModel: [{
                    name: 'comisionId',
                    index: 'comisionId',
                    align: 'center',
        ...
        // jQuery UI Dialogs
        utilDialog.setStandardDialog('#d-removeComision',jQuery('#msgAlertaText').val(),jQuery('#msgDeseaEliminarText').val(), ['Aceptar@comisionJS.remove()', 'Cancelar@']);
        utilDialog.setStandardDialog('#d-removeSeleccionComision', jQuery('#msgAlertaText').val(),jQuery('#msgDeseaEliminarSeleccionText').val(), ['Aceptar@comisionJS.removeSelected()', 'Cancelar@']);
        utilDialog.setStandardDialog('#d-confirmSetEstatusComision', jQuery('#msgAlertaText').val(),jQuery('#msgCambiarEstatusText').val(), ['Aceptar@comisionJS.setEstatusSeleccion()', 'Cancelar@']);
        utilDialog.setStandardDialog('#d-noSelectedComision', jQuery('#msgError').val(),jQuery('#msgSeleccioneUnoOMas').val(), ['Aceptar@']);
        utilDialog.setStandardDialog('#d-registroDuplicadoComision',  'Advertencia',jQuery('#msgYaExisteRegistro').val(), ['Aceptar@']);
...
var comisionJS = {
    ...
    isValidoNombreCallback: function (isValido) {
        if (isValido === true && jQuery('#comisionform_comisionId').val() != 0) {
            var anterior = comisionJS.comision;
            var nuevo= utilObject.buildObject('#comisionform', new ComisionDWR());
            if(anterior.organizacionId === nuevo.organizacionId){ 
                comisionJS.doCommit();
    ...
    exists: function () {
        var comision = utilObject.buildObject('#comisionform', new ComisionDWR());
        ComisionDWR.exists(comision,comisionJS.existsCallback);
    },
...
validanguage.el.comisionform_organizacionId = {
    characters: {
...
validanguage.el.comisionform_valor = {
    ...
    validations: [
        ProfileStripWhitespace, ProfileCheckForInvalidCharacter, {
            ...
            errorMsg: 'Numero no v&aacute;lido. Solo se permiten 13 digitos y 3 decimales',
