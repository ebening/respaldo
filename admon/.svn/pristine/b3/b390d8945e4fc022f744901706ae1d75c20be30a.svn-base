...
public class OrganizacionBssImpl extends BaseBss implements OrganizacionBss {
    ...
    private RecuperarContrasenaBss recuperarContrasenaBss;
    ...
    private OrganizacionUsuario setPassword(OrganizacionUsuario usuario) {
        List<ConfiguracionParametro> password = getParametros("key_password");
        for (ConfiguracionParametro p : password) {
            if(p.getNombre().equals("PASSWORD_DECRYPT_MODE")){
                try {
                    //                usuario.setContrasena(p.getValor());
                    System.out.println("Antes de Encriptar: "+ p.getValor());
                    usuario.setContrasena(Encrypt.encriptar(p.getValor()));
                    System.out.println("Encriptado: "+ usuario.getContrasena());
//                    String pass = Encrypt.encriptar(p.getValor());
//                    System.out.println("Encriptado usando Encriptacion: "+ usuario.getContrasena());
//                    System.out.println("Encriptado usando Encrypt: "+ pass);
                } catch (Exception ex) {
                    Logger.getLogger(OrganizacionBssImpl.class.getName()).log(Level.SEVERE, null, ex);
                }
            }

            if(p.getNombre().equals("PASSWORD_WEBLOGIC_DECRYPT_MODE")){
//                usuario.setContrasenaWeblogic(p.getValor());
                usuario.setContrasenaWeblogic(Encrypt.encriptar(p.getValor()));
            }
    ...
    public void saveOrganizacionUsuario(int userId, int organizacionId,
        ...
        for (OrganizacionUsuario detalle : detalleActualList) {
            ...
            for (OrganizacionUsuario detalleNuevo : detalleNuevoList) {
                detalleNuevo.setContrasena(detalle.getContrasena());
                detalleNuevo.setContrasenaWeblogic(detalle.getContrasenaWeblogic());
     ...
     */
    @Override
    public Organizacion findByIdIgnore(Integer organizacionId) {
        Organizacion organizacion = organizacionDAO.findById(organizacionId);
        if (organizacion == null) {
            return null;
        } else {
            return Arrays.asList(organizacion).get(0);
        }
    }
            ...
            for (Organizacion organizacion : list) {
                ...
                if (organizacion.getModificacionUsuario() != null) {
                    for (Usuario o : usuarioList) {
                        if (o.getUsuarioId().intValue() == organizacion.getModificacionUsuario().intValue()) {
    ...
    public SeguridadPerfilBss getSeguridadPerfilBss() {
        return seguridadPerfilBss;
    }
    ...
    public AplicacionBss getAplicacionBss() {
        return aplicacionBss;
    }
    ...
    public List<Aplicacion> getAplicaciones() {
        return getAplicacionBss().getAplicacionesCombo();
}
    
    @Override
    public List<CatalogoParametro> getPreguntas(Integer catalogoId){
                CatalogoParametro catalogoParametro = new CatalogoParametro();
                catalogoParametro.setCatalogoId(catalogoId);
         return getCatalogoParametroBss().findByCriteriaIgnorePrivacy(catalogoParametro);
    }
    
    @Override
    public void setEstatusUsuario(Integer userId, Integer estatusId, List<Integer> organizacionUsuarioList) {
         organizacionUsuarioBss.setEstatus(userId, estatusId, organizacionUsuarioList);
    }
    
    @Override
    public void removeOrganizacionUsuario(Integer userId, List<Integer> organizacionUsuarioList) {
        organizacionUsuarioBss.delete(userId, organizacionUsuarioList);
}

    @Override
    public void enviaMailOrganizacionUsuario(String usuario) {
        recuperarContrasenaBss.confirmarCuenta(usuario);
    }
