 ...
 * método setter por cada variable que se anexa al final de la clase. */
    ...
    private PaginaAccionBss paginaAccionBss;
 ...
 * @return Regresa una lista con los id's de los registros guardados.
     */
    @Override
    public List<Integer> save(Integer userId, Integer perfilId, List<Pagina> paginaList) {
        DetachedCriteria criteria = createDetachedCriteria();
        criteria.add(Expression.eq("perfilId", perfilId));
        criteria.add(Expression.eq("estatusId", ACTIVO));
        List<Integer> perfilPaginaIdDeleteLista = new ArrayList();
        List<Pagina> perfilPaginaIdActualLista = new ArrayList();
        List<Integer> auxDelete = new ArrayList();
        int cont=0;
        for (PerfilPagina p : findByCriteria(criteria)) {
            auxDelete.add(p.getPerfilPaginaId());
            boolean existe=false;
            cont =0;
            for (Pagina pagina : paginaList) {
                if(pagina.getPaginaId().intValue() == p.getPaginaId()){
                    existe=true;
                    pagina.setPerfilPaginaId(p.getPerfilPaginaId());
                    perfilPaginaIdActualLista.add(pagina);//se almacenan los elementos que ya existían
                    paginaList.remove(cont);//si ya existían los elimino ya que solo se almacenarán los nuevos elementos
                    break;
        }
                cont++;
            }
            if(!existe){
                perfilPaginaIdDeleteLista.add(p.getPerfilPaginaId());//elementos que se deberán eliminar
            }
        }
        //Eliminar paginas
        delete(userId, perfilPaginaIdDeleteLista);
        perfilPaginaPantallaBss.delete(auxDelete);

        List<Integer> savedList = new ArrayList<Integer>();
        //Paginas Nuevas
        for (Pagina pagina : paginaList) {
            PerfilPagina perfilPagina = setParametros(pagina,userId,perfilId);
            savedList.add(perfilPagina.getPerfilPaginaId());
        }
        //Paginas ya existentes solo se guardan funcionalidades
        for (Pagina pagina : perfilPaginaIdActualLista) {
            for (Long funcionalidadId : pagina.getFuncionalidades()) {
                PerfilPaginaPantalla perfilPagPantalla = new PerfilPaginaPantalla();
                perfilPagPantalla.setPerfilPaginaId(pagina.getPerfilPaginaId());
                perfilPagPantalla.setFuncionalidadId(funcionalidadId);
                perfilPaginaPantallaBss.save(userId,Arrays.asList(perfilPagPantalla));
            }
        }
        return savedList;
    }
 ...
 * Setter.
    ...
    public void setPaginaAccionBss(PaginaAccionBss paginaAccionBss) {
        this.paginaAccionBss = paginaAccionBss;
    }
...
* @return regresa una lista de todos los registros de la tabla PAGINA
    ...
    @Override
    public List<String> obtienePaginasFuncionalidades(int perfilId) {
        return perfilPaginaPantallaBss.getPaginaFuncionalidades(perfilId);
    }
    @Override
    public List<PaginaAccion> obtieneFuncionalidades() {
        DetachedCriteria criteria = paginaAccionBss.createDetachedCriteria();
        criteria.add(Expression.eq("estatusId", ACTIVO));
        criteria.add(Expression.eq("eliminadoId", NOELIMINADO));
        return paginaAccionBss.findByCriteria(criteria);
    }
    @Override
    public List<Pagina> obtieneArbol() {
        List<Pagina> listaPaginas = obtienePaginas();
        List<Pagina> listaPaginasFuncionalidades = new ArrayList <Pagina>();
        List<PaginaAccion> listaFuncionalidades = obtieneFuncionalidades();
        for (Pagina pagina : listaPaginas) {
            pagina.setPaginaCompuestaId(pagina.getPaginaId().toString());
            if(pagina.getAnidar()!=0 && pagina.getUrl()!=null){
                for (PaginaAccion paginaAccion : listaFuncionalidades) {
                    Pagina paginaNueva = new Pagina();
                    paginaNueva.setPaginaCompuestaId("_"+pagina.getPaginaId()+"_"+paginaAccion.getFuncionalidadId());
                    paginaNueva.setNombre(paginaAccion.getNombre());
                    paginaNueva.setAnidar(pagina.getPaginaId());
                    paginaNueva.setCreacionUsuario(paginaAccion.getCreacionUsuario());
                    paginaNueva.setCreacionFecha(paginaAccion.getCreacionFecha());
                    paginaNueva.setEliminadoId(paginaAccion.getEliminadoId());
                    paginaNueva.setEstatusId(paginaAccion.getEstatusId());
                    listaPaginasFuncionalidades.add(paginaNueva);
                }
            }
        }
        listaPaginas.addAll(listaPaginasFuncionalidades);
        return listaPaginas;
    }
...
* @return regresa una lista de todos los registros de la tabla PAGINA
    ...
    public PerfilPagina setParametros(Pagina pagina,Integer userId, Integer perfilId){
        int perfilPaginaId ;
        PerfilPagina perfilPagina = new PerfilPagina();
        perfilPagina.setPerfilId(perfilId);
        perfilPagina.setPaginaId(pagina.getPaginaId());
        perfilPagina.setEstatusId(ACTIVO);
        ...
        perfilPagina.setModificacionFecha(new Timestamp(new Date().getTime()));
        perfilPaginaId = perfilPaginaDAO.save(perfilPagina);
        for (Long funcionalidadId : pagina.getFuncionalidades()) {
            PerfilPaginaPantalla perfilPagPantalla = new PerfilPaginaPantalla();
            perfilPagPantalla.setPerfilPaginaId(perfilPaginaId);
            perfilPagPantalla.setFuncionalidadId(funcionalidadId);
            perfilPaginaPantallaBss.save(userId,Arrays.asList(perfilPagPantalla));
}
        return perfilPagina;
