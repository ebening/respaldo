jQuery(document).ready(function () {
    ...
    // Actualizar "Presentacion" al seleccionar "Evento" en formulario estadoCuenta
    jQuery('#estadoCuentaform_eventoId').change(function () {
        estadoCuentaJS.filtrarPresentacionEvento();
    });
    ...
    utilObject.getFuncionalidadPantalla("","EstadoCuentaAction");
    ...
    jQuery.extend($.fn.fmatter , {
            currencyFmatter : function(cellvalue, options, rowdata) {
                var valor = new Number(cellvalue);
    ...
    if (jQuery('#gridVisibleEstadoCuenta').val() == 'true') {
        jQuery('#estadoCuentaGrid').jqGrid({
            ...
            colNames: [
                ...
                jQuery('#key_estadocuenta_nombrePresentacion').val(),
...
//        estadoCuentaJS.reloadGrid();
        ...
        utilGrid.setHideElementsTable('estadoCuentaGridPagerId',['refresh','activate','search', 'export', 'delete']);
    }
...
//        console.info(data);
     ...
     * 
     */
    filtrarPresentacionEvento: function () {
        utilEffect.showProgressBar();
        EstadoCuentaDWR.filtrarPresentacionEvento(
                jQuery('#estadoCuentaform_eventoId').val(),
                estadoCuentaJS.filtrarPresentacionEventoCallback);
    },
    /** 
     * Callback de la función filtrarEvento(), esta
     * función es llamada por DWR al terminar de realizar la consulta.
     */
    filtrarPresentacionEventoCallback: function (data) {
//        console.info(data);
        if (data == null || data.length == 0) {
            utilInput.removeOptionsSelectAndAddDummy('#estadoCuentaform_presentacionId', '--Seleccione--');
        } else {
            utilInput.populateSelect('#estadoCuentaform_presentacionId', data, 'presentacionId', 'nombre');
            if (estadoCuentaJS.estadoCuenta !== null) {
                if (estadoCuentaJS.estadoCuenta.presentacionId !== null) {
                    if (utilInput.existsOptionByValueInSelect('#estadoCuentaform_eventoId', estadoCuentaJS.estadoCuenta.presentacionId)) {
                        jQuery('#estadoCuentaform_presentacionId').val(estadoCuentaJS.estadoCuenta.presentacionId);
                    } else {
                        jQuery('#estadoCuentaform_presentacionId').val(0);
                    }
                }
            }
            jQuery('#estadoCuentaform_presentacionId').trigger('change');
        }
        jQuery('#estadoCuentaform_presentacionId').val(-1);
        utilEffect.hideProgressBar();
    },
    /** 
    ...
    exportarExcel: function () {
        utilEffect.showProgressBar();
        var estadoCuenta = utilObject.buildObject('#estadoCuentaform', new EstadoCuentaDWR());
        estadoCuenta.ivaKey= jQuery('#key_iva').val();
        estadoCuenta.configuracionId= jQuery('#key_configuracionId').val();
